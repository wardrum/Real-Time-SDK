...
class RestReactor {
    ...
    public RestReactor(RestReactorOptions options, ReactorErrorInfo errorInfo) {
        ...
        try {
            ...
            final ConnectionConfig connectionConfig = ConnectionConfig.custom().setBufferSize(options.bufferSize())
                    .setFragmentSizeHint(options.fragmentSizeHint()).build();

            _ioEventDispatch = new DefaultHttpClientIODispatch<HttpAsyncRequestExecutor>(_protocolHandler, sslContext,
                    connectionConfig);

            _ioReactor = new DefaultConnectingIOReactor(IOReactorConfig.custom()
                    .setConnectTimeout(options.connectTimeout()).setSoTimeout(options.soTimeout())
                    .setTcpNoDelay(options.tcpNoDelay()).setIoThreadCount(options.ioThreadCount())
                    .setSoKeepAlive(options.soKeepAlive()).setSelectInterval(options.selectInterval())
                    .setShutdownGracePeriod(options.shutdownGracePeriod()).build());
            ...
            populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.initialize",
                    "failed to initialize the SSLConnectionSocketFactory, exception=" + getExceptionCause(e));
            return;
        } catch (IOReactorException e) {
            populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.initialize",
                    "failed to initialize the DefaultConnectingIOReactor, exception=" + getExceptionCause(e));
            return;
        }
    ...
    public int submitAuthRequest(RestAuthOptions authOptions, final RestConnectOptions restConnectOptions,
            ReactorAuthTokenInfo authTokenInfo, final ReactorErrorInfo errorInfo, boolean redirect, String location) {
    ...
    public int submitAuthRequest(RestAuthOptions authOptions, final RestConnectOptions restConnectOptions,
        ...
        if (!_reactorActive) {
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.submitAuthRequest",
        ...
        if (authOptions.grantType().equals(AUTH_REFRESH_TOKEN)) // for new refresh token
        {
            ...
            // must set for the first access_token, otherwise receive status code: 403
            // forbidden.
            // must not include scope if the scope for reissue is same, client will issue
            // new token in the same scope.
        } else {
            params.add(new BasicNameValuePair(AUTH_TAKE_EXCLUSIVE_SIGN_ON_CONTROL,
                    authOptions.takeExclusiveSignOnControlAsString()));
        ...
        if ((restConnectOptions.proxyHost() == null || restConnectOptions.proxyHost().isEmpty())
            ...
            new Thread() {
                public void run() {
                    ...
                    HttpHost proxy = new HttpHost(restConnectOptions.proxyHost(), restConnectOptions.proxyPort(),
                            "http");
    ...
    public int submitRequestForServiceDiscovery(RestRequest request, RestConnectOptions restConnectOptions,
            ReactorAuthTokenInfo authTokenInfo, List<ReactorServiceEndpointInfo> reactorServiceEndpointInfoList,
            final ReactorErrorInfo errorInfo, boolean redirect, String location) {
    ...
    public int submitRequestForServiceDiscovery(RestRequest request, RestConnectOptions restConnectOptions,
        ...
        if (!_reactorActive) {
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.submitRequest",
                    "RestReactor is not active, aborting");
        }
        ...
        try {
            ...
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE,
                    "RestReactor.submitRequestForServiceDiscovery",
                    "failed to submit a request, exception = " + getExceptionCause(e));
        }
        ...
        String url = null;
        try {
            if ((restConnectOptions.discoveryRedirect()) && (restConnectOptions.discoveryRedirectLocation() != null)) {
    ...
    public int dispatch(ReactorErrorInfo errorInfo) {
        if (!_reactorActive) {
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.dispatch",
                    "RestReactor is not active, aborting");
        }
        ...
        try {
            ...
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.dispatch",
                    "received Interrupted IOException, exception = " + getExceptionCause(e));
        } catch (final IOException e) {
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.dispatch",
                    "received IOException, exception = " + getExceptionCause(e));
        }
    ...
    public int shutdown(ReactorErrorInfo errorInfo) {
        ...
        try {
            ...
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.shutdown",
                    "received IOException, exception = " + getExceptionCause(e));
        }
    ...
    public int submitAuthRequestBlocking(RestAuthOptions authOptions, RestConnectOptions restConnectOptions,
            ReactorAuthTokenInfo authTokenInfo, ReactorErrorInfo errorInfo) throws IOException {
        if (!_reactorActive) {
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.submitAuthRequest",
                    "RestReactor is not active, aborting");
        }
        ...
        if (_sslconSocketFactory == null)
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.submitAuthRequestBlocking",
                    "failed to initialize the SSLConnectionSocketFactory");
        ...
        try {
            ...
            if (authOptions.hasHeaderAttribute()) {
                ...
                if (headers.containsKey(HttpHeaders.TRANSFER_ENCODING)
                        && headers.get(HttpHeaders.TRANSFER_ENCODING).contentEquals(HTTP.CHUNK_CODING))
            ...
            // Initial value of URL is either default one or saved permanent redirect
            // location.
            ...
            while ((attemptCount <= 1) && (!done)) {
                ...
                if ((restConnectOptions.proxyHost() != null && !restConnectOptions.proxyHost().isEmpty())
                    ...
                    RequestConfig config = RequestConfig.custom().setProxy(proxy)
                            .setSocketTimeout(_restReactorOptions.soTimeout()).build();
                    httppost.setConfig(config);
                    ...
                    final CloseableHttpClient httpClient = HttpClientBuilder.create()
                            .setSSLSocketFactory(_sslconSocketFactory).build();
                    RequestConfig config = RequestConfig.custom().setSocketTimeout(_restReactorOptions.soTimeout())
                            .build();
                    httppost.setConfig(config);
                    try {
                        ...
                        if (loggerClient.isTraceEnabled()) {
                            loggerClient
                                    .trace(prepareResponseString(response, contentString, extractingContentException));
                        }

                        int statusCode = response.getStatusLine().getStatusCode();
                        switch (statusCode) {
                        case HttpStatus.SC_OK:
                            convertResponse(this, response, restResponse, errorInfo, contentString,
            ...
            if ((attemptCount > 1) && (!done)) {
                populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.submitAuthRequestBlocking",
                        "Failed to request authentication token information. " + "Too many redirect attempts.");

                return ReactorReturnCodes.FAILURE;
            }
        } catch (JSONException | ParseException | IOException e) {
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestReactor.submitAuthRequestBlocking",
                    "failed to submit authorization request, exception = " + getExceptionCause(e));
        }

        return ReactorReturnCodes.SUCCESS;
    }

    public int submitServiceDiscoveryRequestBlocking(RestRequest request, RestConnectOptions restConnectOptions,
            ReactorAuthTokenInfo authTokenInfo, List<ReactorServiceEndpointInfo> reactorServiceEndpointInfoList,
        ...
        try {
            ...
            if ((attemptCount > 1) && (!done)) {
                populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE,
                        "RestReactor.submitServiceDiscoveryRequestBlocking",
                        "Failed to request service discovery information. " + "Too many redirect attempts.");
    ...
    static int convertResponse(RestReactor RestReactor, HttpResponse response, RestResponse clientResponse,
        ...
        if (extractingContentException != null) {
            return populateErrorInfo(errorInfo, ReactorReturnCodes.FAILURE, "RestHandler.handleResponse",
                    "failed to convert entity to json object, exception = "
